          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`snap-start shrink-0 flex items-center gap-2 px-6 py-2.5 rounded-xl border transition-all duration-300 outline-none
                  ${isActive
                    ? 'border-primary bg-primary/10 shadow-[0_0_20px_rgba(242,208,13,0.25)]'
                    : 'border-white/10 bg-white/5 hover:bg-white/10'
                  }`}
              >
                <span className={`text-xs font-bold tracking-widest uppercase ${isActive ? 'text-primary' : 'text-white/60'}`}>{cat}</span>
              </button>
            );
          })}
        </div>
